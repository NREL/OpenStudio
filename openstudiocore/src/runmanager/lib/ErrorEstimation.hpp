  ...
  namespace runmanager {
    ...
    class RUNMANAGER_API ErrorEstimation
    {
      public:
        ...
        ErrorEstimation(const std::string &t_baselineSourceName);
        ...
        FuelUses add(const SqlFile &t_sql, const std::string &t_sourceName, const std::string &t_variable, 
        ...
        FuelUses estimate(const std::string &t_variable, const double t_value) const;

        /// Returns the error, by fuel type for the given conditions
        FuelUses getError(const std::string &t_sourceName, const std::string &t_variable, const double t_value) const;

      private:
        std::string m_baselineSourceName;
        std::map<std::string, std::map<double, std::map<std::string, FuelUses> > > m_data;
