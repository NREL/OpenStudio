...
namespace detail {
  ...
  void Job_Impl::standardClean()
  {
    ...
    std::set<openstudio::path> outputfiles;
         ...
         ++itr)
    {
      ...
      std::vector<std::pair<QUrl, openstudio::path> > required = itr->requiredFiles;

      for (std::vector<std::pair<QUrl, openstudio::path> >::const_iterator itr = required.begin();
           itr != required.end();
           ++itr)
      {
        openstudio::path r = openstudio::toPath(itr->first.toLocalFile());
        if (!r.is_complete())
        {
          r = outpath / r;
        }
    ...
    for (QFileInfoList::const_iterator itr = list.begin();
         itr != list.end();
         ++itr)
    {
      QString extension = itr->suffix().toLower();

      openstudio::path foundpath = openstudio::toPath(itr->absoluteFilePath());
          ...
          && outputfiles.find(foundpath.replace_extension(openstudio::toPath(".osm").string())) == outputfiles.end())
      {
        if (!itr->isDir())
        {
          LOG(Debug, "standardClean: " << openstudio::toString(itr->absoluteFilePath()));

          if (!dir.remove(itr->absoluteFilePath()))
          {
            LOG(Error, "standardClean: error removing file: " << openstudio::toString(itr->absoluteFilePath()));
          }
        ...
        LOG(Debug, "standardClean skipping output file: " << openstudio::toString(itr->absoluteFilePath()));
      }
    }
  ...
  void Job_Impl::maximumClean()
  {
         ...
         ++itr)
    {
      ...
      if (itr->fileName().startsWith("out", Qt::CaseInsensitive)
          || extension == "osm"
          || extension == "idf"
          || extension == "sql"
          || extension == "xml"
      ...
      {
        LOG(Debug, "maximumClean: skipping file: " << openstudio::toString(itr->absoluteFilePath()));
      } else {
        if (!itr->isDir())
        {
          LOG(Debug, "maximumClean: " << openstudio::toString(itr->absoluteFilePath()));

          if (!dir.remove(itr->absoluteFilePath()))
          {
            LOG(Error, "maximumClean: error removing file: " << openstudio::toString(itr->absoluteFilePath()));
          }
  ...
  Tools Job_Impl::allChildTools() const
  {
    ...
    QReadLocker l(&m_mutex);
    std::vector<boost::shared_ptr<Job_Impl> >::const_iterator itr = m_children.begin(), end = m_children.end();
    while (itr != end)
    {
      ret.append((*itr)->tools());
      ++itr;
    }
  ...
  Files Job_Impl::allChildFiles() const
  {
    ...
    while (itr != end)
    {
      ret.append((*itr)->inputFiles());
      ret.append((*itr)->outputFiles());
      ret.append((*itr)->allChildFiles());
      ++itr;
    }
  ...
  boost::optional<QDateTime> Job_Impl::childrenLastRun() const
  {
    ...
    for (std::vector<boost::shared_ptr<Job_Impl> >::const_iterator itr = m_children.begin();
        ...
        ++itr)
    {
      boost::optional<QDateTime> childlastrun = (*itr)->lastRun();
  ...
  void Job_Impl::showErrors() const
  {
    ...
    JobErrorsDisplay jed(0,Qt::WindowFlags(Qt::Dialog | Qt::WindowTitleHint),err);
    jed.exec();
  }
...
//    LOG(Debug, description() << " " << outofdateinternal << " " << parentOutOfDate << " " << parentRunning << " " << parentChildrenRunning << " " << parentRunning << " " << parentFailed << " " << parentChildrenFailed)
  ...
  bool Job_Impl::treeRunning() const
  {
    ...
    for (std::vector<boost::shared_ptr<Job_Impl> >::const_iterator itr = m_children.begin();
         ...
         ++itr)
    {
      if ((*itr)->treeRunning()) { return true; }
    }
  ...
  void Job_Impl::setTreeCanceled(bool t_canceled)
  {
    ...
    for (std::vector<boost::shared_ptr<Job_Impl> >::const_iterator itr = m_children.begin();
         ...
         ++itr)
    {
      (*itr)->setTreeCanceled(t_canceled);
    }
  ...
  Files Job_Impl::treeOutputFiles() const
  {
    ...
    for (std::vector<boost::shared_ptr<Job_Impl> >::const_iterator itr = m_children.begin();
        ...
        ++itr)
    {
      files.append((*itr)->treeOutputFiles());
    }
  ...
  TreeStatusEnum Job_Impl::treeStatus() const
  {
    ...
    for (std::vector<boost::shared_ptr<Job_Impl> >::const_iterator itr = m_children.begin();
         ...
         ++itr)
    {
      res = compareTreeStatus(res, (*itr)->treeStatus());
    }
  ...
  std::vector<std::string> Job_Impl::treeDetailedDescription() const
  {
    ...
    for (std::vector<boost::shared_ptr<Job_Impl> >::const_iterator itr = m_children.begin();
         ...
         ++itr)
    {
      std::vector<std::string> childresults = (*itr)->treeDetailedDescription();
  ...
  boost::optional<QDateTime> Job_Impl::treeLastRun() const
  {
    ...
    for (std::vector<boost::shared_ptr<Job_Impl> >::const_iterator itr = m_children.begin();
        ...
        ++itr)
    {
      boost::optional<QDateTime> childlastrun = (*itr)->treeLastRun();
  ...
  {
    for (std::map<openstudio::path, FileTrack>::const_iterator itr = t_files.begin();
         itr != t_files.end();
         ++itr)
    {
      QFileInfo qfi(openstudio::toQString(itr->first));
      if (qfi.lastModified() >= t_lastRun)
      {
        std::string checksum = openstudio::checksum(itr->first);
        if (checksum != itr->second.checksum)
        {
          return true;
        }
      }
