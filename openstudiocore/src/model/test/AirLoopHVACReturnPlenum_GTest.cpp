...
TEST_F(ModelFixture,AirLoopHVACReturnPlenum)
{
  ...
  {  
     ...
     model::AirLoopHVACReturnPlenum plenum(m); 

     exit(0); 
  } ,
    ::testing::ExitedWithCode(0), "" );

  // Add to system 7, on node directly after terminal
  {
    Model m;

    AirLoopHVACReturnPlenum plenum(m); 
  ...
  // Make sure that other nodes fail
  {
    ...
    Node demandOutletNode = system7.demandOutletNode();

    ASSERT_FALSE(plenum.addToNode(demandOutletNode));
  }
  ...
  // Make sure that system branches that already have a plenum wont accept another plenum
  {
    ...
    AirLoopHVACReturnPlenum plenum2(m); 
