...
module Kernel
  ...
  def require path
    ...
    # If ends by `json/ext` we, throw. Could probably just do
    # `if path.strip == 'json/ext'`
    if path[/json\/ext$/]
      if RUBY_PLATFORM =~ /mswin/
        raise LoadError.new("Preventing segfault if native JSON is built as MinGW (tried to require #{path}).")
      end
    end
