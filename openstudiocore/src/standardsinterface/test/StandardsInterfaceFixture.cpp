...
// static variables
boost::optional<openstudio::FileLogSink> StandardsInterfaceFixture::logFile;
openstudio::path StandardsInterfaceFixture::runDir;

void StandardsInterfaceFixture::SetUp() {}

void StandardsInterfaceFixture::TearDown() {}

void StandardsInterfaceFixture::SetUpTestCase() {
  // set up logging  // set up logging
  logFile = FileLogSink(toPath("./StandardsInterfaceFixture.log"));
  logFile->setLogLevel(Debug);
  openstudio::Logger::instance().standardOutLogger().disable();
}

void StandardsInterfaceFixture::TearDownTestCase() {
  logFile->disable();
  exampleModel(false,true); // should call SqlFile destructor
  boost::filesystem::remove_all(runDir);
}

openstudio::model::Model StandardsInterfaceFixture::exampleModel(bool withResults, bool clearModels) {
  ...
  if (withResults) {
    if (modelWithResults.numObjects() == 0u) {
      ...
      runDir = resourcesPath() / toPath("standardsinterface/ExampleModel_" + toString(createUUID()));
