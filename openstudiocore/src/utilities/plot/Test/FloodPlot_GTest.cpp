...
TEST(FloodPlot, TimeSeriesFloodPlot_Detailed_Year_212plus10)
{
  ...
  DateTimeVector dateTimes(8760);
  Vector values(8760);
  DateTime startDate(Date(MonthOfYear::Jul, startDay), Time(0,startHour,0,0));
  for(unsigned i = 0; i < 8760; ++i){
    dateTimes[i] = startDate + Time(0,i,0,0);
    values[i] = i % 24;
  }
  ...
  EXPECT_DOUBLE_EQ(0.0, data->minValue());
  EXPECT_DOUBLE_EQ(23.0, data->maxValue());
  EXPECT_DOUBLE_EQ(0.0, data->value(212,10.0));
  EXPECT_DOUBLE_EQ(1.0, data->value(212,10.5));
  EXPECT_DOUBLE_EQ(2.0, data->value(212,12.0-epsilon));
  EXPECT_DOUBLE_EQ(0.0, data->value(212,5.0));
  EXPECT_DOUBLE_EQ(0.0, data->value(1.0,1.0-epsilon)); 
  EXPECT_DOUBLE_EQ(12.0, data->value(215.0,22.0-epsilon)); 
  EXPECT_DOUBLE_EQ(14.0, data->value(216.0,23.0)); 
  EXPECT_DOUBLE_EQ(0.0, data->value(2.0,23.0-epsilon));
  EXPECT_DOUBLE_EQ(0.0, data->value(0,25.0-epsilon)); 
  EXPECT_DOUBLE_EQ(0.0, data->value(0,24.0-epsilon));
  EXPECT_DOUBLE_EQ(0.0, data->value(0,2.0));
...
TEST(FloodPlot, TimeSeriesFloodPlot_Detailed_Day)
{
  ...
  Vector values(24);
  DateTime startDate(Date(MonthOfYear::Jun, startDay), Time(0,startHour,0,0));
  ...
  EXPECT_DOUBLE_EQ(0.0, data->minValue());
  EXPECT_DOUBLE_EQ(23.0, data->maxValue());
  EXPECT_DOUBLE_EQ(15.0, data->value(153,0.0));
