...
QuantityConverterSingleton::QuantityConverterSingleton()
{
  ...
  const baseUnitConversionFactor toSI[] =
  {
    { UnitSystem::SI, "kg", "kg", 1.0, 0 },
  ...
  const baseUnitConversionFactor fromSI[] =
  {
    { UnitSystem::IP, "kg", "lb_m", 2.20462262185, 0 },
  ...
  while( !(fromSI[i].originalUnit.empty() && fromSI[i].targetUnit.empty()) ) {
    this->m_fromSIBySystemMap.insert(
        UnitSystemConversionMultiMap::value_type(fromSI[i].system, fromSI[i]));
    ++i;
  }
}
