...
*
*
* OpenStudio SDD Translator is a conversion utility between SDD data files and OpenStudio Model (osm files).  The translation is supported in both directions.  The utility that converts from SDD format to the OpenStudio Model is called SDD ReverseTranslator.  Similarly, SDD ForwardTransltor is the utility that converts from an OpenStudio Model to SDD format.  The conversion from SDD to OpenStudio is comprehensive, encompasing all aspects of the building which relate to energy modeling, including geometry, loads, and HVAC.  Translation from OpenStudio to SDD is partial, suporting only the geometric aspects of the building.
*
...
*
* ForwardTransltor.hpp defines the complete set of transltor functions for converting an OpenStudio Model to an SDD file.  Translation is invoked by calling ForwardTranslate::modelToSDD(), passing the model to translate and the path to save the SDD file.
*
*/
