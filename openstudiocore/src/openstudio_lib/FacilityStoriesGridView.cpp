...
namespace openstudio {
    ...
    m_isIP(isIP)
  {
    ...
    m_gridController = new FacilityStoriesGridController(m_isIP, "Space Types", IddObjectType::OS_SpaceType, model, spaceTypeModelObjects);
  ...
  std::vector<model::ModelObject> FacilityStoriesGridView::selectedObjects() const
  {
    return m_gridController->selectedObjects();
  }

  void FacilityStoriesGridView::enableFilter()
  {
    m_filterLabel->setEnabled(true);
    m_filters->setEnabled(true);
  }

  void FacilityStoriesGridView::disableFilter()
  {
