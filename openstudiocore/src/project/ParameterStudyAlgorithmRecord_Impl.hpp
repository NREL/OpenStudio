...
namespace detail {
  ...
  class PROJECT_API ParameterStudyAlgorithmRecord_Impl : public DakotaAlgorithmRecord_Impl {
   public:
    ...
    ParameterStudyAlgorithmRecord_Impl(
        const analysis::ParameterStudyAlgorithm& parameterStudyAlgorithm,
        AnalysisRecord& analysisRecord);

    /** Constructor from query. Throws if bad query. */
    ParameterStudyAlgorithmRecord_Impl(const QSqlQuery& query, ProjectDatabase& database);

    virtual ~ParameterStudyAlgorithmRecord_Impl() {}
   ...
   private:
    ...
    analysis::ParameterStudyAlgorithmType m_algorithmType;

    analysis::ParameterStudyAlgorithmType m_lastAlgorithmType;
  };
...
#endif // PROJECT_PARAMETERSTUDYALGORITHMRECORD_IMPL_HPP
