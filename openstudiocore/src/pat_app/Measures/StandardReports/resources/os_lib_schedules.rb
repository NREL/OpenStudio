module OsLib_Schedules
  ...
  def OsLib_Schedules.timeConditionalScheduleValueAdjust(model,schedule,hhmm_before,hhmm__after,inside_double,outside_double,modificationType = "Multiplier")# can increase/decrease by percentage or static value
    ...
    profiles.each do |day_sch|
      ...
      for i in 0..(values.length - 1)
        ...
        if times[i] > before_time and times[i] <= after_time # updated this so times[i] == before_time goes into the else
  ...
  def OsLib_Schedules.adjust_hours_of_operation_for_schedule_ruleset(runner,model,schedule,options = {})
    ...
    rules.each do |rule|
      ...
      alter_rule = false
      if rule.applyMonday and rule.applyMonday == options["mon"] then alter_rule = true end
      if rule.applyTuesday and rule.applyTuesday == options["tue"] then alter_rule = true end
      if rule.applyWednesday and rule.applyWednesday == options["wed"] then alter_rule = true end
      if rule.applyThursday and rule.applyThursday == options["thur"] then alter_rule = true end
      if rule.applyFriday and rule.applyFriday == options["fri"] then alter_rule = true end
      if rule.applySaturday and rule.applySaturday == options["sat"] then alter_rule = true end
      if rule.applySunday and rule.applySunday == options["sun"] then alter_rule = true end
    ...
    if options["base_start_hoo"] <= options["base_finish_hoo"]
      ...
      if mid_non_hoo > time_24 then mid_non_hoo = mid_non_hoo - time_24 end
    else
    ...
    profiles.each do |day_sch|
      ...
      times.each do |time|
        ...
        if !found_24_or_0 and new_time <= min_time_new
