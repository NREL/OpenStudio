    ...
    OtherEquipment& modelObject)
{
  ...
  if (space){
    ...
    if (thermalZone){
      idfObject.setString(OtherEquipmentFields::ZoneName, thermalZone->name().get());
    }
  }else if(spaceType){
    idfObject.setString(OtherEquipmentFields::ZoneName, spaceType->name().get());
  }
  ...
  if (d){
    idfObject.setDouble(OtherEquipmentFields::WattsperZoneFloorArea, (*d)*multiplier);
  }

  d = definition.wattsperPerson();
  if (d){
    idfObject.setDouble(OtherEquipmentFields::WattsperPerson, (*d)*multiplier);
  }
