...
{
public:
    SpeciesPrivate(int nr=0,int sflag=0,int ntflag=0,RX molwt=RX_INIT(0),RX mdiam=RX_INIT(0),RX edens=RX_INIT(0),RX decay=RX_INIT(0),RX Dm=RX_INIT(0),RX ccdef=RX_INIT(0),RX Cp=RX_INIT(0),int ucc=0,int umd=0,int ued=0,int udm=0,int ucp=0,STRING name=STRING_INIT,STRING desc=STRING_INIT);
    ...
    STRING name;  // species name (CS)
    STRING desc;  // species description (CS) {W}
};

class AhsPrivate : public QSharedData
{
public:
    AhsPrivate(int nr=0,int zone_r=0,int zone_s=0,int path_r=0,int path_s=0,int path_x=0,STRING name=STRING_INIT,STRING desc=STRING_INIT);
    void read(Reader &reader);
    STRING write();
    ...
    STRING name;  // AHS name (CS)
    STRING desc;  // AHS description (CS) {W} may be blank
};
...
public:
    ...
    STRING cfd_name;  // cfd path id (CS)
...
public:
    ...
    STRING WTHpath;  // full name of weather file (CS)
    STRING CTMpath;  // full name of contaminant file (CS)
    STRING CVFpath;  // full name of continuous values file (CS)
    STRING DVFpath;  // full name of discrete values file (CS)
    STRING WPCfile;  // full name of WPC file (CS)
    STRING EWCfile;  // full name of EWC data source file (CS) {W}
    STRING WPCdesc;  // WPC description (CS) {W}
    ...
    STRING tShift;  // time shift of EWC data {W} [s] (hh:mm:ss) {W}
    STRING dStart;  // date WPC data starts (mm/dd) {W}
    STRING dEnd;  // date WPC data ends (mm/dd) {W}
    ...
    STRING date_st;  // day-of-year to start steady simulation (mmmdd)
    STRING time_st;  // time-of-day to start steady simulation (hh:mm:ss)
    STRING date_0;  // day-of-year to start transient simulation (mmmdd)
    STRING time_0;  // time-of-day to start transient simulation (hh:mm:ss)
    STRING date_1;  // day-of-year to end transient simulation (mmmdd)
    STRING time_1;  // time-of-day to end transient simulation (hh:mm:ss)
    STRING time_step;  // simulation time step [s] (hh:mm:ss)
    STRING time_list;  // simulation output (results) time step [s] (hh:mm:ss)
    STRING time_scrn;  // simulation status time step [s] (up to 1 day) (hh:mm:ss)
    int restart;  // use restart file (I2)
    STRING rstdate;  // restart date (mmmdd)
    STRING rsttime;  // restart time (hh:mm:ss)
...
{
public:
    LevelPrivate(int nr=0,RX refht=RX_INIT(0),RX delht=RX_INIT(0),int u_rfht=0,int u_dlht=0,STRING name=STRING_INIT,std::vector<Icon> icons=std::vector<Icon>());
    void read(Reader &reader);
    STRING write();
...
{
public:
    DaySchedulePrivate(int nr=0,int shape=0,int utyp=0,int ucnv=0,STRING name=STRING_INIT,STRING desc=STRING_INIT,std::vector<SchedulePoint> points=std::vector<SchedulePoint>());
    void read(Reader &reader);
    STRING write();
    ...
    STRING name;  // schedule name (CS) {W}
    STRING desc;  // schedule description (CS) {W} may be blank
...
{
public:
    WeekSchedulePrivate(int nr=0,int utyp=0,int ucnv=0,STRING name=STRING_INIT,STRING desc=STRING_INIT,std::vector<int> j=std::vector<int>());
    void read(Reader &reader);
    STRING write();
...
{
public:
    WindPressureProfilePrivate(int nr=0,int type=0,STRING name=STRING_INIT,STRING desc=STRING_INIT,std::vector<PressureCoefficientPoint> coeffs=std::vector<PressureCoefficientPoint>());
    void read(Reader &reader);
    STRING write();
    int nr;  // profile number (IX); in order from 1 to _nwpf
    int type;  // 1 = linear; 2 = cubic spline; 3 = trigonometric (I2)
    STRING name;  // schedule name (CS) {W}
    STRING desc;  // profile description (CS) {W} may be blank
...
public:
    ...
    STRING desc;  // control node description (CS) {W} may be blank
};
...
public:
    ...
    STRING units;  // units of coordinates {W} (CS)
    STRING species;  // species name [CS]; convert to pointer
};
...
public:
    ...
    STRING name;  // element name (CS) {W}
    STRING desc;  // control node description (CS) {W} may be blank
    STRING valuename;  // name of the value read from the Discrete or Continuous Values file (CS)
};
...
public:
    ...
    STRING header;  // header string (CS)
    STRING units;  // units string (CS)
};
