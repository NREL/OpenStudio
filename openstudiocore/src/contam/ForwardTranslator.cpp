...
InfiltrationTranslator::InfiltrationTranslator() : ForwardTranslator()
{
}

bool InfiltrationTranslator:: translate(const openstudio::model::Model& model,openstudio::path epwPath,
  openstudio::path wthPath, ProgressBar *progressBar)
{
  if(ForwardTranslator::translate(model,false,progressBar))
  {
    return translateEpw(epwPath,wthPath);
  }
  return false;
}
