...
namespace contam {
  ...
  boost::optional<QString> ForwardTranslator::translateToPrj(const openstudio::model::Model& model)
  {
    ...
    if (!building){
      //name = QString("OpenStudio Model");
      descr = QString("Automatically generated OpenStudio model");
    }
    else{
      name = QString::fromStdString(building->name().get());
      descr = QString("Automatically generated from \"%1\" OpenStudio model").arg(name);
    }
