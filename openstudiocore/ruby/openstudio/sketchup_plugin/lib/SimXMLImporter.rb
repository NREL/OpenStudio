...
module OpenStudio
   ...
   class SimXMLImporter < Sketchup::Importer
     ...
     def id
       return "com.sketchup.importers.SimXML"
     end
     ...
     def load_file(file_path, status)
       ...
       Plugin.write_pref("Last SimXML Import Dir", File.dirname(file_path))  # Save the dir so we can start here next time

       openstudio_model = Plugin.model_manager.model_from_SimXML_path(file_path)
       if not openstudio_model
         return(0)
       end
       
       if (Plugin.command_manager.prompt_to_continue_import("SimXML"))         
   ...
   Sketchup.register_importer(SimXMLImporter.new)
