...
class OpenStudioBuildRecipe(ConanFile):
    ...
    def requirements(self):
        ...
        if is_apple_os(self):
            self.requires("boost/1.79.0", force=True, options={'visibility': 'global'})  #f664bfe40e2245fa9baf1c742591d582
        else:
            self.requires("boost/1.79.0", force=True)
        self.requires("pugixml/1.12.1")  #5a39f82651eba3e7d6197903a3202e21
        self.requires("libxml2/[<2.12.0]") # deprecated xmlSubstituteEntitiesDefault and co https://github.com/GNOME/libxml2/commit/db8b9722cb2a1f7dca7374ec38ecaa4936ab3869
        ...
        self.requires("minizip/1.2.13")  #0b5296887a2558500d0323c6c94c8d02 # This depends on zlib, and basically patches it
        self.requires("zlib/[>=1.2.11 <2]")  #3b9e037ae1c615d045a06c67d88491ae # Also needed, so we can find zlib.h and co (+ pinning exactly is good)
