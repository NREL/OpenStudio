...
class ProjectClassGenerator < SubProjectClassGenerator
  ...
  def cppConstructors()
    ...
    if @joinRecord
      ...
      result << "  {\n"
      result << "    Q_ASSERT(" << @leftArgument << ".projectDatabase().handle() == " 
      ...
      result << "  {\n"
      result << "    Q_ASSERT(query.isValid());\n"
      result << "    Q_ASSERT(query.isActive());\n"
      result << "    Q_ASSERT(query.isSelect());\n\n"
      ...
      result << "    value = query.value(" << @className << "::ColumnsType::" << @classArgument 
          result << "Type);\n"
      result << "    Q_ASSERT(value.isValid() && !value.isNull());\n"
  ...
  def cppPublicClassConstructors()
    ...
    if @joinRecord
      ...
      result << "{\n"
      result << "  Q_ASSERT(getImpl<detail::" << @className << "_Impl>());\n"
      result << "}\n\n"
  ...
  def cppPublicClassProtectedMethods()
    ...
    if pImpl
      ...
      result << "{\n"
      result << "  Q_ASSERTgetImpl<detail::" << @className << "_Impl>());\n"
      result << "}\n"
    end
