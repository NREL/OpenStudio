  ...
  def implHppQMacros()
    ...
    if @idfObject
      ...
      @nonextensibleFields.each { |field|
        ...
        # object list fields define relationships, not attributes
        next if field.isObjectList?
        
        result << "\n"      
          
        result << "    Q_PROPERTY(" << field.getterReturnType << " " << field.getterName 
        result << " READ " << field.getterName << " WRITE " << field.setterName 
        if field.hasReset? 
          result << " RESET " << field.resetName 
        end
        result  << ");\n"
        
        if field.isReal?
          result << "    Q_PROPERTY(" << field.quantityPropertyType << " " << field.getterName << "_SI"
          result << " READ " << field.getterName << "_SI WRITE " << field.setterName
          if field.hasReset?
            result << " RESET " << field.resetName
          end
          result << ");\n"
          
          result << "    Q_PROPERTY(" << field.quantityPropertyType << " " << field.getterName << "_IP"
          result << " READ " << field.getterName << "_IP WRITE " << field.setterName
          if field.hasReset?
            result << " RESET " << field.resetName
          end
          result << ");\n"          
        ...
        if field.hasDefault?
          result << "    Q_PROPERTY(bool " << field.isDefaultName << " READ " 
          result << field.isDefaultName << ");\n"
        end

        if field.canAutosize?
          result << "    Q_PROPERTY(bool " << field.isAutosizeName << " READ " 
          result << field.isAutosizeName << ");\n"
        end

        if field.canAutocalculate?
          result << "    Q_PROPERTY(bool " << field.isAutocalculateName << " READ " 
          result << field.isAutocalculateName << ");\n"
        end
        
        if field.isChoice? and not field.isBooleanChoice?
          result << "    Q_PROPERTY(std::vector<std::string> " << field.getterName << "Values READ "
          result << field.getterName << "Values);\n"
        end
      }
      
      # Relationships
      result << "\n"      
      result << "    // TODO: Add relationships for objects related to this one, but not pointed to by the underlying data.\n"
      result << "    //       Such relationships can be generated by the GenerateRelationships.rb script.\n"
      @nonextensibleFields.each { |field|
        next if not field.isObjectList?
        
        result << "    Q_PROPERTY(boost::optional<openstudio::model::ModelObject> " << field.getterName 
        result << " READ " << field.getterName << "AsModelObject WRITE " << field.setterName << "AsModelObject"
        if field.hasReset? 
          result << " RESET " << field.resetName 
        end
        result  << ");\n"
    ...
    if @idfObject
      ...
      @nonextensibleFields.each { |field|
        if field.isReal?
          ...
          result << "    " << field.quantityPropertyType << " " << field.getterName << "_SI() const;\n"
          result << "    " << field.quantityPropertyType << " " << field.getterName << "_IP() const;\n"
        
          any = true
          
        elsif field.isChoice? and not field.isBooleanChoice?
        
          result << "\n" if not any
          

          result << "    std::vector<std::string> " << field.getterName << "Values() const;\n"
      ...
      @nonextensibleFields.each { |field| 
        ...
        result << "    boost::optional<ModelObject> " << field.getterName << "AsModelObject() const;\n"
      ...
      @nonextensibleFields.each { |field| 
        ...
        result << "    bool " << field.setterName << "AsModelObject(const boost::optional<ModelObject>& modelObject);\n"
      }
    ...
    if @idfObject
      ...
      @nonextensibleFields.each { |field|
        if field.isReal?
          ...
          result << "  " << field.quantityPropertyType << " " << @className << "_Impl::" << field.getterName << "_SI() const {\n"
          result << "    return " << field.quantityGetterName << "(false);\n"
          result << "  }\n\n"
        
          result << "  " << field.quantityPropertyType << " " << @className << "_Impl::" << field.getterName << "_IP() const {\n"
          result << "    return " << field.quantityGetterName << "(true);\n"
          result << "  }\n\n"
         
        elsif field.isChoice? and not field.isBooleanChoice?
        
          result << "  std::vector<std::string> " << @className << "_Impl::" << field.getterName << "Values() const {\n"
          result << "    return " << @className << "::" << field.getterName << "Values();\n"
          result << "  }\n\n"
      ...
      @nonextensibleFields.each { |field| 
        ...
        result << "  boost::optional<ModelObject> " << @className << "_Impl::"  << field.getterName << "AsModelObject() const {\n"
        if field.optionalGetter?
          result << "    OptionalModelObject result;\n"
          result << "    Optional" << field.objectListClassName << " intermediate = " << field.getterName << "();\n"
          result << "    if (intermediate) {\n"
          result << "      result = *intermediate;\n"
          result << "    }\n"
        else
          result << "    OptionalModelObject result = " << field.getterName << "();\n"
        end
        result << "    return result;\n"
        result << "  }\n\n"
      }
      ...
      @nonextensibleFields.each { |field| 
        ...
        objectListClassName = field.objectListClassName
     
        result << "  bool " << @className << "_Impl::" << field.setterName << "AsModelObject(const boost::optional<ModelObject>& modelObject) {\n"
        result << "    if (modelObject) {\n"
        result << "      Optional" << objectListClassName << " intermediate = modelObject->optionalCast<" << objectListClassName << ">();\n"
        result << "      if (intermediate) {\n"
        result << "        " << objectListClassName << " " << field.setterArgumentName << "(*intermediate);\n"
        result << "        return " << field.setterName << "(" << field.setterArgumentName << ");\n"
        result << "      }\n"
        if field.hasReset?
          result << "      else {\n"
          result << "        return false;\n"
