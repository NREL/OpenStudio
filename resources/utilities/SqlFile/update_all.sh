...
for ep_version in "${all_versions[@]}"; do
  ...
  tar_gz_link=$(curl https://api.github.com/repos/NREL/EnergyPlus/releases/tags/v$ep_version | jq -r '.assets | .[] | select(.name | contains("Linux")) | select(.name | contains("tar.gz")) | .browser_download_url')
  if [ -z "$tar_gz_link" ]; then
    sh_link=$(curl https://api.github.com/repos/NREL/EnergyPlus/releases/tags/v$ep_version | jq -r '.assets | .[] | select(.name | contains("Linux")) | select(.name | contains("sh")) | .browser_download_url')
    # sh_name=${sh_link##*/}
