...
TEST_F(EnergyPlusFixture, ForwardTranslator_AirConditionerVariableRefrigerantFlow) {
  ...
  EXPECT_EQ(scheduleConstant.nameString(),
            idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::AvailabilityScheduleName, false).get());
  ...
  EXPECT_EQ(curve1.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::CoolingCapacityRatioModifierFunctionofLowTemperatureCurveName, false).get());
  ...
  EXPECT_EQ(curve3.nameString(), idf_vrf.getDouble(AirConditioner_VariableRefrigerantFlowFields::CoolingCapacityRatioModifierFunctionofHighTemperatureCurveName, false).get());
  EXPECT_EQ(curve4.nameString(), idf_vrf.getDouble(AirConditioner_VariableRefrigerantFlowFields::CoolingEnergyInputRatioModifierFunctionofLowTemperatureCurveName, false).get());
  EXPECT_EQ(curve5.nameString(), idf_vrf.getDouble(AirConditioner_VariableRefrigerantFlowFields::CoolingEnergyInputRatioBoundaryCurveName, false).get());
  EXPECT_EQ(curve6.nameString(), idf_vrf.getDouble(AirConditioner_VariableRefrigerantFlowFields::CoolingEnergyInputRatioModifierFunctionofHighTemperatureCurveName, false).get());
  EXPECT_EQ(curve7.nameString(), idf_vrf.getDouble(AirConditioner_VariableRefrigerantFlowFields::CoolingEnergyInputRatioModifierFunctionofLowPartLoadRatioCurveName, false).get());
  EXPECT_EQ(curve8.nameString(), idf_vrf.getDouble(AirConditioner_VariableRefrigerantFlowFields::CoolingEnergyInputRatioModifierFunctionofHighPartLoadRatioCurveName, false).get());
  EXPECT_EQ(curve9.nameString(), idf_vrf.getDouble(AirConditioner_VariableRefrigerantFlowFields::CoolingCombinationRatioCorrectionFactorCurveName, false).get());
  EXPECT_EQ(curve10.nameString(), idf_vrf.getDouble(AirConditioner_VariableRefrigerantFlowFields::CoolingPartLoadFractionCorrelationCurveName, false).get());
  ...
  EXPECT_EQ(curve11.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatingCapacityRatioModifierFunctionofLowTemperatureCurveName, false).get());
  EXPECT_EQ(curve12.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatingCapacityRatioBoundaryCurveName, false).get());
  EXPECT_EQ(curve13.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatingCapacityRatioModifierFunctionofHighTemperatureCurveName, false).get());
  EXPECT_EQ(curve14.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatingEnergyInputRatioModifierFunctionofLowTemperatureCurveName, false).get());
  EXPECT_EQ(curve15.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatingEnergyInputRatioBoundaryCurveName, false).get());
  EXPECT_EQ(curve16.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatingEnergyInputRatioModifierFunctionofHighTemperatureCurveName, false).get());
  EXPECT_EQ("WetBulbTemperature", idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatingPerformanceCurveOutdoorTemperatureType, false).get());
  EXPECT_EQ(curve17.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatingEnergyInputRatioModifierFunctionofLowPartLoadRatioCurveName, false).get());
  EXPECT_EQ(curve18.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatingEnergyInputRatioModifierFunctionofHighPartLoadRatioCurveName, false).get());
  EXPECT_EQ(curve19.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatingCombinationRatioCorrectionFactorCurveName, false).get());
  EXPECT_EQ(curve20.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatingPartLoadFractionCorrelationCurveName, false).get());
  ...
  EXPECT_EQ(scheduleConstant2.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::ThermostatPriorityScheduleName, false).get());
  EXPECT_EQ(vrf.nameString() + " Terminal List", idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::ZoneTerminalUnitListName, false).get());
  EXPECT_EQ("No", idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatPumpWasteHeatRecovery, false).get());
  EXPECT_EQ(11, idf_vrf.getDouble(AirConditioner_VariableRefrigerantFlowFields::EquivalentPipingLengthusedforPipingCorrectionFactorinCoolingMode, false).get());  
  ...
  EXPECT_EQ(curve21.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::PipingCorrectionFactorforLengthinCoolingModeCurveName, false).get());
  EXPECT_EQ(13, idf_vrf.getDouble(AirConditioner_VariableRefrigerantFlowFields::PipingCorrectionFactorforHeightinCoolingModeCoefficient, false).get());  
  EXPECT_EQ(14, idf_vrf.getDouble(AirConditioner_VariableRefrigerantFlowFields::EquivalentPipingLengthusedforPipingCorrectionFactorinHeatingMode, false).get());  
  EXPECT_EQ(curve22.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::PipingCorrectionFactorforLengthinHeatingModeCurveName, false).get());
  EXPECT_EQ(15, idf_vrf.getDouble(AirConditioner_VariableRefrigerantFlowFields::PipingCorrectionFactorforHeightinHeatingModeCoefficient, false).get());  
  ...
  EXPECT_EQ(curve23.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::DefrostEnergyInputRatioModifierFunctionofTemperatureCurveName, false).get());
  ...
  EXPECT_EQ(scheduleConstant3.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::BasinHeaterOperatingScheduleName, false).get());
  ...
  EXPECT_EQ(curve24.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatRecoveryCoolingCapacityModifierCurveName, false).get());
  ...
  EXPECT_EQ(curve25.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatRecoveryCoolingEnergyModifierCurveName, false).get());
  ...
  EXPECT_EQ(curve26.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatRecoveryHeatingCapacityModifierCurveName, false).get());
  ...
  EXPECT_EQ(curve27.nameString(), idf_vrf.getString(AirConditioner_VariableRefrigerantFlowFields::HeatRecoveryHeatingEnergyModifierCurveName, false).get());
