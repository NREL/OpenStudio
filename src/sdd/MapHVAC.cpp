  ...
  openstudio::model::Model& model)
{
  ...
  else if( istringEqual(coilHeatingTypeElement.text().as_string(),"HeatPump") )
  {
    ...
    if (openstudio::istringEqual(cndsrTypeElement.text().as_string(), "WaterSource"))
    {
      ...
      auto heatingCapacityCurve = coil.heatingCapacityCurve().optionalCast<model::CurveQuadLinear>();
      if( heatingCapacityCurve )
      {
      ...
      auto heatingPowerCurve = coil.heatingPowerConsumptionCurve().optionalCast<model::CurveQuadLinear>();
      if( heatingPowerCurve )
      {
        heatingPowerCurve->setCoefficient1Constant(-2.176941116);
        heatingPowerCurve->setCoefficient2w(0.832114286);
        heatingPowerCurve->setCoefficient3x(1.570743399);
        heatingPowerCurve->setCoefficient4y(0.690793651);
        heatingPowerCurve->setCoefficient5z(0.0);
      }
    }
    else
    {
  ...
  openstudio::model::Model& model)
{
  ...
  const auto makeTable = [](double val75, double val100, const model::Model& model, const std::string& tableName) {
    ...
    model::TableLookup tableLookup(model);
  ...
  auto _htgSensEff100 = htgSensEff100Element.text().as_double(0.76);

  // HtgSensEff75
  auto htgSensEff75Element = element.child("HtgSensEff75");
  auto _htgSensEff75 = htgSensEff75Element.text().as_double(0.81);

  auto senseHeatingTable = makeTable(_htgSensEff75, _htgSensEff100, model, name + "_SensibleHeatingEff");
  hx.setSensibleEffectivenessofHeatingAirFlowCurve(senseHeatingTable);

  // HtgLatEff100
  auto htgLatEff100Element = element.child("HtgLatEff100");
  auto _htgLatEff100 = htgLatEff100Element.text().as_double(0.68);

  // HtgLatEff75
  auto htgLatEff75Element = element.child("HtgLatEff75");
  auto _htgLatEff75 = htgLatEff75Element.text().as_double(0.73);

  auto latentHeatingTable = makeTable(_htgLatEff75, _htgLatEff100, model, name + "_LatentHeatingEff");
  hx.setLatentEffectivenessofHeatingAirFlowCurve(latentHeatingTable);

  // ClgSensEff100
  auto clgSensEff100Element = element.child("ClgSensEff100");
  auto _clgSensEff100 = clgSensEff100Element.text().as_double(0.76);

  // ClgSensEff75
  auto clgSensEff75Element = element.child("ClgSensEff75");
  auto _clgSensEff75 = clgSensEff75Element.text().as_double(0.81);

  auto sensibleCoolingTable = makeTable(_clgSensEff75, _clgSensEff100, model, name + "_SensibleCoolingEff");
  hx.setSensibleEffectivenessofCoolingAirFlowCurve(sensibleCoolingTable);

  // ClgLatEff100
  auto clgLatEff100Element = element.child("ClgLatEff100");
  auto _clgLatEff100 = clgLatEff100Element.text().as_double(0.68);

  // ClgLatEff75
  auto clgLatEff75Element = element.child("ClgLatEff75");
  auto _clgLatEff75 = clgLatEff75Element.text().as_double(0.73);
  ...
  openstudio::model::Model& model)
{
  ...
  if( istringEqual(coilCoolingTypeElement.text().as_string(),"DirectExpansion") )
  {
    ...
    if( istringEqual(cndsrTypeElement.text().as_string(),"WaterSource") )
    {
      ...
      auto coolingCapacityCurve = coil.totalCoolingCapacityCurve().cast<model::CurveQuadLinear>();
      coolingCapacityCurve.setCoefficient1Constant(-9.149069561);
      coolingCapacityCurve.setCoefficient2w(10.87814026);
      coolingCapacityCurve.setCoefficient3x(-1.718780157);
      coolingCapacityCurve.setCoefficient4y(0.746414818);
      coolingCapacityCurve.setCoefficient5z(0.0);

      auto sensibleCoolingCapacityCurve = coil.sensibleCoolingCapacityCurve().cast<model::CurveQuintLinear>();
      ...
      auto coolingPowerCurve = coil.coolingPowerConsumptionCurve().cast<model::CurveQuadLinear>();
      coolingPowerCurve.setCoefficient1Constant(-3.20456384);
      coolingPowerCurve.setCoefficient2w(-0.976409399);
      coolingPowerCurve.setCoefficient3x(3.97892546);
      coolingPowerCurve.setCoefficient4y(0.938181818);
      coolingPowerCurve.setCoefficient5z(-0.0);
    ...
    openstudio::model::ThermalZone& thermalZone)
{
  ...
  else if( istringEqual(type,"WSHP") )
  {
    ...
    if( schedule && fan && heatingCoil && coolingCoil && suppHeatingCoil ) {
      ...
      const auto _htPumpFanDelay = element.child("HtPumpFanDelay").text().as_double(60.0);

      auto coolingCoil = wshp->coolingCoil();
      if (auto c_eq_ = coolingCoil.optionalCast<model::CoilCoolingWaterToAirHeatPumpEquationFit>()) {
        c_eq_->setMaximumCyclingRate(2.5);
        c_eq_->setLatentCapacityTimeConstant(60.0);
        c_eq_->setFanDelayTime(_htPumpFanDelay);
      } else if (auto c_vs_ = coolingCoil.optionalCast<model::CoilCoolingWaterToAirHeatPumpVariableSpeedEquationFit>()) {
        c_vs_->setMaximumCyclingRate(2.5);
        c_vs_->setLatentCapacityTimeConstant(60.0);
        c_vs_->setFanDelayTime(_htPumpFanDelay);
      }
...
boost::optional<pugi::xml_node> ForwardTranslator::translateCoilCoolingDXSingleSpeed(const openstudio::model::CoilCoolingDXSingleSpeed& coil, pugi::xml_node& airSegElement)
{
  ...
  // DXEER
  auto r = 0.12;
  auto eer = (coil.ratedCOP() * (1 - r)  - r) * 3.413;
