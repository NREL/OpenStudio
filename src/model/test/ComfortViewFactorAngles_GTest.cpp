...
TEST_F(ModelFixture, ComfortViewFactorAngles) {
  ...
  EXPECT_THROW(ComfortViewFactorAngle(surface, -0.01), openstudio::Exception);
  EXPECT_THROW(ComfortViewFactorAngle(surface, 1.01), openstudio::Exception);

  EXPECT_TRUE(comfortViewFactorAngles.addComfortViewFactorAngle(surface, 0.25));
  ...
  EXPECT_FALSE(comfortViewFactorAngles.addComfortViewFactorAngle(otherSurface, 0.75));
  EXPECT_EQ(1u, comfortViewFactorAngles.numberofComfortViewFactorAngles());
